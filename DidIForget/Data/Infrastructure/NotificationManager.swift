...
final class NotificationManager: NSObject {
    ...
    func scheduleExitNotification(for reminder: ExitReminder) {
        let content = UNMutableNotificationContent()
        content.title = "Did I Forget?"
        content.body = reminder.title
        content.sound = .default
        content.categoryIdentifier = "EXIT_REMINDER"
    ...
    func cancelNotification(for reminder: ExitReminder) {
        notificationCenter.removePendingNotificationRequests(withIdentifiers: [reminder.regionIdentifier])
        notificationCenter.removeDeliveredNotifications(withIdentifiers: [reminder.regionIdentifier])
    }
    ...
    func scheduleDelayedNotification(for reminder: ExitReminder, delay: TimeInterval) {
        let content = UNMutableNotificationContent()
        content.title = "Did I Forget?"
        content.body = reminder.title
        content.sound = .default
        ...
        let request = UNNotificationRequest(
            identifier: "debug_\(reminder.regionIdentifier)",
            content: content,
            trigger: trigger
        )
